...
module.exports = class weChat {
  ...
  * getAccessToken() {
    ...
    if (wxAccessToken !== null && wxAccessToken.hasOwnProperty('expires_time') && wxAccessToken.expires_time > this.app.getNowSysTime()) {
  ...
  * getJsapiTicket(accessToken) {
    let jsapiTicket = yield this.app.redis.get('wx_jsapi_ticket');
    jsapiTicket = JSON.parse(jsapiTicket);
    if (jsapiTicket !== null && jsapiTicket.hasOwnProperty('expires_time') && jsapiTicket.expires_time > this.app.getNowSysTime()) {
      return {
        errcode: 0,
        ticket: jsapiTicket.jsapi_ticket,
      };
    }
    ...
    if (res.hasOwnProperty('errcode') && res.errcode === 0) {
      ...
      yield this.app.redis.set('wx_jsapi_ticket', JSON.stringify(jsapiData));
    }
    return res;
  }
  * getSignJs(jsapiTicket, url) {
    const ret = yield sign(jsapiTicket, url);
    return ret;
  }
  // 菜单相关
  /**
  ...
  * getUserInfoList(openidList, accessToken) {
   ...
   * @param accessToken
   */
  * getUserOpenidList(nextOpenid, accessToken) {
  ...
  * addMaterial(type, filePath, accessToken, videoDes = {}) {
   ...
   * @returns {*}
   */
  * addTempMaterial(type, filePath, accessToken,) {
